...
static void sha256_transform(asper_sha256_ctx *ctx, const uint8_t *block) {
...
void asper_sha256_final(asper_sha256_ctx *ctx, uint8_t out[32]) {
  ...
  if (!ctx || !out) return;
  bits = ctx->bitlen;
  asper_sha256_update(ctx, &pad80, 1);
  while (ctx->buflen != 56) asper_sha256_update(ctx, &zero, 1);
  for (i = 0; i < 8; i++) tail[i] = (uint8_t)(bits >> (56 - 8 * i));
  asper_sha256_update(ctx, tail, 8); /* flushes the final block */
  for (i = 0; i < 8; i++) {
...
void asper_sha256(const void *data, size_t len, uint8_t out[32]) {
  ...
  asper_sha256_update(&ctx, data, len);
  asper_sha256_final(&ctx, out);
}
...
asper_err asper_sha256_file(const char *path, uint8_t out[32]) {
  ...
  if (!path || !out) return ASPER_ERR_INVALID;
  f = os_fopen(path, "rb");
  if (!f) return os_file_exists(path) ? ASPER_ERR_IO : ASPER_ERR_NOT_FOUND;
  buf = malloc(65536);
  if (!buf) {
    fclose(f);
  ...
  while ((got = fread(buf, 1, 65536, f)) > 0)
    asper_sha256_update(&ctx, buf, got);
