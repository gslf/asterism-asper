 ...
 * ASPER_NO_THREADS is defined — the no-op locking/threading set. The
 * platform backends (os_posix.c / os_win32.c) compile their threading
 * sections only when ASPER_NO_THREADS is NOT defined, so exactly one
...
asper_err os_write_file(const char *path, const void *data, size_t len)
{
    ...
    if (e == ASPER_OK)
        e = os_fsync(f);
    if (fclose(f) != 0 && e == ASPER_OK)
        e = ASPER_ERR_IO;
    return e;
}

/* ---- ASPER_NO_THREADS: no-op locking, no worker thread ------------------ */
