...
typedef void (*asper_test_fn)(void);
typedef struct {
  const char *name;
  asper_test_fn fn;
} asper_test_case;

static int asper_test_case_failed = 0;

#define TEST(name) static void name(void)
#define TEST_LIST static const asper_test_case asper_test_cases[]
...
#define ASPER_FAILF(...)                                    \
  do {                                                      \
    ...
    fputc('\n', stderr);                                    \
    asper_test_case_failed = 1;                             \
  } while (0)

#define ASSERT_TRUE(cond)                                   \
  do {                                                      \
    if (!(cond)) {                                          \
      ASPER_FAILF("ASSERT_TRUE(%s)", #cond);                \
      return;                                               \
    }                                                       \
  } while (0)
...
#define ASSERT_EQ_INT(actual, expected)                                     \
  do {                                                                      \
    ...
    if (a_ != e_) {                                                         \
      ASPER_FAILF("ASSERT_EQ_INT(%s, %s): %lld != %lld", #actual,           \
                  #expected, a_, e_);                                       \
      return;                                                               \
    }                                                                       \
  } while (0)
...
#define ASSERT_EQ_DBL(actual, expected, eps)                                \
  do {                                                                      \
    ...
    if (!(fabs(a_ - e_) <= (double)(eps))) {                                \
      ASPER_FAILF("ASSERT_EQ_DBL(%s, %s): %.17g != %.17g (eps %g)",         \
...
#define ASSERT_OK(expr)                                                     \
  do {                                                                      \
    asper_err e_ = (expr);                                                  \
    if (e_ != ASPER_OK) {                                                   \
      ASPER_FAILF("ASSERT_OK(%s): %s", #expr, asper_err_name(e_));          \
      return;                                                               \
    }                                                                       \
  } while (0)
...
#define ASSERT_ERR(expr, want)                                              \
  do {                                                                      \
    asper_err e_ = (expr);                                                  \
    if (e_ != (want)) {                                                     \
      ASPER_FAILF("ASSERT_ERR(%s): got %s, want %s", #expr,                 \
                  asper_err_name(e_), asper_err_name(want));                \
      return;                                                               \
    }                                                                       \
  } while (0)
...
static void asper_test_rmtree(const char *path) {
#if defined(_WIN32)
...
static char *asper_test_read_file(const char *path, size_t *out_len) {
...
#define RUN_ALL_TESTS()                                                     \
  int main(void) {                                                          \
    size_t i_, n_ = sizeof(asper_test_cases) / sizeof(asper_test_cases[0]); \
    int failed_ = 0;                                                        \
    (void)asper_test_tmpdir;                                                \
    (void)asper_test_rmtree;                                                \
    (void)asper_test_read_file;                                             \
    for (i_ = 0; i_ < n_; i_++) {                                           \
      asper_test_case_failed = 0;                                           \
      fprintf(stderr, "[ RUN  ] %s\n", asper_test_cases[i_].name);          \
      asper_test_cases[i_].fn();                                            \
      if (asper_test_case_failed) {                                         \
        failed_++;                                                          \
