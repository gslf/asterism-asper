...
- **content**: string. Self-contained natural-language statement, understandable without the conversation that produced it. Max `content_max_chars` (default 500).
...
1. Acquire the write lock; flush pending access batch.
2. Serialize the current table (active + not-yet-purged deprecated records) to `<file>.tmp` per section, fsync.
3. Atomically replace each target file (`rename(2)` on POSIX, `ReplaceFileW` on Windows).
4. Purge: deprecated records past `purge_after` are simply not written.
5. Reset the journal (write empty temp, atomic replace), update `manifest.last_compaction`, release the lock.
...
Each record renders as a `"- "` line containing content verbatim. A section that contributes zero records is removed together with its heading; if nothing at all is injected, the base prompt is returned unchanged. Rendering is fully deterministic: identical store + identical inputs ⇒ byte-identical prompt.
...
- `content_max_chars` (default 500): longer `INSERT`/`UPDATE` text is rejected (the instruction states the cap; rejection keeps memories atomic).
...
- One readers-writer lock guards the in-memory table + vector index. Read-only calls (`asper_build_prompt`, `asper_memory_search`, `asper_memory_list`, `asper_get_stats`) take the read lock; every mutation — curator op, API op, MCP op — funnels through a single internal `apply()` that takes the write lock and holds the journal mutex for the append.
...
- Recall (§7.7) shares the curator context and therefore the worker: requests execute on the worker between cycles, ahead of queued curation, while the caller blocks on a completion event with timeout. In `ASPER_NO_THREADS` builds, `asper_recall` runs the cycle synchronously on the caller.
...
The host callback (`asper_set_logger`) is independent of the file sink and receives every record above its own threshold; the default callback writes to stderr at `warn`. The journal remains the readable audit trail of applied ops until compaction, and `storage.audit_log: true` additionally mirrors every op — including rejected ones — to `audit.xcdn`, which is never compacted.
